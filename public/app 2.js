...
function renderFleet(){
  ...
  grid.innerHTML=fleet.map(m=>{
    ...
    return `<article class="machine-card" data-machine="${escapeHtml(m.id)}">
      <div class="machine-card-head"><div><p class="section-kicker">${escapeHtml(kind.toUpperCase())} · ${escapeHtml(m.id)}</p><h3>${escapeHtml(m.name||m.id)}</h3><div class="loc">${escapeHtml(m.location||'Ubicación sin definir')}</div></div><span class="machine-status ${m.online?'online':''}"></span></div>
      <div class="machine-card-metrics"><div><span>Hoy</span><b>${money(t.totalToday)}</b></div><div><span>Mes</span><b>${money(t.totalMonth)}</b></div><div><span>Histórico</span><b>${money(t.totalHistoric)}</b></div><div><span>WiFi</span><b>${t.wifiRssi??'—'} dBm</b></div></div>
      <div class="machine-card-foot"><span>${m.online?'ONLINE':'OFFLINE'}</span><span>Abrir →</span></div>
    </article>`;
  }).join('');
...
function renderMachine(m){
  ...
  text('wifi',`${t.wifiRssi??'—'} dBm`);text('machineType',kind);text('espStatus',m.online?'Conectado':'Sin conexión');
  text('lastSeen',m.updatedAt?new Date(m.updatedAt).toLocaleString('es-CL'):'Sin datos');
  const wb=$('wifiBadge');if(wb)wb.textContent=t.wifiRssi>-65?'Excelente':t.wifiRssi>-78?'Buena':'Débil';
  ...
  text('bills',bills===null?'—':money(bills));text('coins',coins===null?'—':money(coins));text('cashCurrent',cash===null?'—':money(cash));
...
async function command(type,payload={}){
  if(!selectedMachineId)return toast('Selecciona una máquina');
